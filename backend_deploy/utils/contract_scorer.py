...
RED_FLAGS: list[tuple[str, str, int, str]] = [
    ...
    # Add to RED_FLAGS in utils/contract_scorer.py
(r"shall\s+be\s+(considered\s+)?final", "MEDIUM", 12, "Decisions framed as unappealable — no recourse mechanism"),
(r"may\s+be\s+(reassigned|relocated|transferred)", "MEDIUM", 8, "Broad relocation/reassignment rights with minimal employee protection"),
(r"discretionary", "MEDIUM", 8, "Pay/benefits framed as entirely at employer's discretion"),
(r"reasonably\s+reachable", "LOW", 5, "Implies unpaid availability outside working hours"),
(r"may\s+be\s+(monitored|recorded)", "LOW", 4, "Broad monitoring clause — check scope and consent"),
(r"recoverable\s+upon", "MEDIUM", 8, "Clawback clause — costs may be deducted from final pay"),
(r"six\s+months?\s+notice", "MEDIUM", 10, "Unusually long notice period heavily favors employer"),
